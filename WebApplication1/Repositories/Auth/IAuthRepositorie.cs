...
namespace WebApplication1.Repositories.Auth
{
    public interface IAuthRepositorie
    {
        ...
        Task SignInWithClaimsAsync(ApplicationUser user, bool isPersistent, string firstName, string lastName, string workspaceId);
        Task<IList<string>> GetUserRolesAsync(ApplicationUser user);
        Task<IdentityResult> CreateUserAsync(ApplicationUser user, string password);
        Task<bool> RoleExistsAsync(string roleName);
        Task<IdentityResult> AddToRoleAsync(ApplicationUser user, string roleName);
        Task SignOutAsync();
        Task<int?> CreateWorkspaceAsync(WorkspaceSetupViewModel model, string ownerId);
        Task<string> GetWorkspaceIdByOwnerIdAsync(string ownerId);
    }
}
